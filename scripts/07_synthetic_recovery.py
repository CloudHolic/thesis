...
def main() -> None:
	...
	ap.add_argument("--steps", type=int, default=None, help="overrides [train]; for timing runs")
	args = ap.parse_args()
	...
	center = marginal.find_center(transform.to_tau(jnp.asarray(fit.z)), split)
	usable = int(np.asarray(center.usable).sum())

	print(
		f"map       {fit.steps} steps in {map_seconds:.1f}s, "
		f"first {fit.seconds[0]:.1f}s incl. compile, then {1000 * step_seconds:.1f} ms/step, "
		f"converged={fit.converged}, adaptive centres {usable:,}/{n_persons:,}"
	)
